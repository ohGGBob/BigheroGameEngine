...
{
  public:
    BinaryReader(const uint8_t* data, std::size_t size) : data_(data), size_(size), pos_(0) {}
    explicit BinaryReader(const std::vector<uint8_t>& v) : data_(v.data()), size_(v.size()), pos_(0) {}
