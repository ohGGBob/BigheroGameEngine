...
{
  public:
    FbmNoise(uint32_t seed = 1337u, int octaves = 5, float persistence = 0.5f, float lacunarity = 2.0f)
        : seed_(seed), octaves_(octaves > 0 ? octaves : 1), persistence_(persistence), lacunarity_(lacunarity)
    {
    }
  ...
  private:
    ...
    {
        uint32_t n = static_cast<uint32_t>(x) * 374761393u + static_cast<uint32_t>(y) * 668265263u + seed * 974634631u;
