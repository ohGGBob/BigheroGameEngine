...
class Plane3
{
  public:
    Plane3() = default;
    Plane3(float nx, float ny, float nz, float d) : nx_(nx), ny_(ny), nz_(nz), d_(d) { Normalize(); }
    ...
    float SignedDistance(float px, float py, float pz) const { return nx_ * px + ny_ * py + nz_ * pz - d_; }
    float AbsDistance(float px, float py, float pz) const { return std::fabs(SignedDistance(px, py, pz)); }
