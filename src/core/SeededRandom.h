...
class SeededRandom
{
  public:
    ...
    void Reseed(uint64_t seed) { state_ = seed ? seed : 0x9E3779B97F4A7C15ULL; }
    ...
    // Float in [0,1).
    float NextFloat() { return (float)(NextU64() >> 40) / (float)(1u << 24); }
    ...
    bool Bool(float prob = 0.5f) { return NextFloat() < prob; }
