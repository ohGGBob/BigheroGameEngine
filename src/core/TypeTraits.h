...
// 是否可拷贝构造/赋值
template<typename T> inline constexpr bool IsCopyable = std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>;
// 是否可移动构造/赋值
template<typename T> inline constexpr bool IsMovable = std::is_move_constructible_v<T> && std::is_move_assignable_v<T>;
// 是否默认可构造
template<typename T> inline constexpr bool IsDefaultConstructible = std::is_default_constructible_v<T>;
// 是否算术类型且非 bool
template<typename T> inline constexpr bool IsArithmetic = std::is_arithmetic_v<T> && !std::is_same_v<T, bool>;
// 是否指针
