...
namespace BigHero::Core
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            T& Get(uint32_t entityIndex) noexcept
            {
                return dense_[sparse_[entityIndex]].value;
            }
            const T& Get(uint32_t entityIndex) const noexcept
            {
                return const_cast<SparseSet*>(this)->Get(entityIndex);
            }
    ...
    public:
        ...
        {
            Pool<T>().RemoveEntity(e.Index());
        }

        // 组件池访问（供 View 使用）。公开以便 View 驱动迭代。
        template <typename T>
        detail::SparseSet<T>& Pool()
        {
    ...
    private:
        bool ContainsAll(const uint32_t idx) const
        {
            return ContainsAllImpl(idx, std::index_sequence_for<Ts...>{});
        }
        template <size_t... Is>
        bool ContainsAllImpl(const uint32_t idx, std::index_sequence<Is...>) const
        {
            return (std::get<Is>(pools_).Contains(idx) && ...);
        }
        template <typename F, size_t... Is>
        void Call(F& fn, const uint32_t idx, std::index_sequence<Is...>)
        {
