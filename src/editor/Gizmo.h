...
namespace BigHero::Editor
{
    ...
    inline glm::vec2 ProjectWorldToScreen(const glm::vec3& world, const glm::mat4& viewProj,
    ...
    inline GizmoAxis PickAxis(const glm::vec3& originWorld, const glm::mat4& viewProj,
        const glm::vec2& viewport, const glm::vec2& mouse, float thresholdPx, float armPx = 80.0f)
    {
        ...
        {
            const glm::vec2 tip = ProjectWorldToScreen(originWorld + GizmoAxisVector(ax),
                viewProj, viewport);
            if (tip.x < -1e8f)
                continue;
    ...
    inline float TranslateDragDelta(const glm::vec3& originWorld, GizmoAxis axis,
        const glm::mat4& viewProj, const glm::vec2& viewport, const glm::vec2& mouseDeltaPx)
    {
    ...
    inline float RotateDragAngle(const glm::vec2& centerScreen, const glm::vec2& fromMouse,
        const glm::vec2& toMouse)
    {
