...
namespace BigHero
{
    void CubeShadowMap::Create(const Context& ctx, uint32_t size)
    {
        ...
        const VkFormat depthFormat = FindSupportedFormat(ctx.PhysicalDevice(),
        ...
        depthImage_.Create(ctx, size_, size_, depthFormat,
            ...
            /*arrayLayers=*/6, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT,
        ...
        for (int face = 0; face < kFaceCount; ++face)
        {
            ...
            fbInfo.height = size_;
            fbInfo.layers = 1;
            VK_CHECK(vkCreateFramebuffer(ctx.Device(), &fbInfo, nullptr, &framebuffers_[face]),
                "创建立方体阴影帧缓冲");
