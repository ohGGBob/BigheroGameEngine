...
namespace BigHero
{
    namespace
    {
        ...
        VkImageView createFaceView(VkDevice device, VkImage image, VkFormat format,
    ...
    glm::vec3 EnvironmentLighting::SampleSky(glm::vec3 dir)
    {
        ...
        glm::vec3 sky = (h > 0.0f)
            ? glm::mix(horizon, zenith, std::pow(h, 0.45f))
            : glm::mix(horizon, ground, std::pow(-h, 0.35f));
        ...
        const float sunGlow = std::pow(glm::max(cosSun, 0.0f), 200.0f) * 3.0f
            + std::pow(glm::max(cosSun, 0.0f), 16.0f) * 0.35f;
    ...
    void EnvironmentLighting::Create(const Context& ctx)
    {
        ...
        envCubemap_.Create(ctx, kEnvSize, kEnvSize, VK_FORMAT_R32G32B32A32_SFLOAT,
            ...
            1, VK_SAMPLE_COUNT_1_BIT, 6, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT,
                ...
                for (uint32_t y = 0; y < kEnvSize; ++y)
                {
                    for (uint32_t x = 0; x < kEnvSize; ++x)
                    {
                        ...
                        const glm::vec3 dir = glm::normalize(
                            fb.major + fb.sVec * (2.0f * s - 1.0f) + fb.tVec * (2.0f * t - 1.0f));
        ...
        for (uint32_t face = 0; face < 6; ++face)
        {
            const VkImageView view = createFaceView(device, irradianceCubemap_.Get(),
                VK_FORMAT_R16G16B16A16_SFLOAT, 0, face);
        ...
        for (uint32_t mip = 0; mip < kPrefilterMips; ++mip)
        {
            ...
            for (uint32_t face = 0; face < 6; ++face)
            {
                const VkImageView view = createFaceView(device, prefilteredCubemap_.Get(),
                    VK_FORMAT_R16G16B16A16_SFLOAT, mip, face);
        ...
        ctx.SubmitOneTime([&](VkCommandBuffer cmd) {
            ...
            vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                irradiancePipe.GetLayout(), 0, 1, &envSet_, 0, nullptr);
            for (uint32_t face = 0; face < 6; ++face)
            {
                ...
                vkCmdPushConstants(cmd, irradiancePipe.GetLayout(), VK_SHADER_STAGE_VERTEX_BIT,
                    0, sizeof(PushFace), &push);
            ...
            for (uint32_t mip = 0; mip < kPrefilterMips; ++mip)
            {
                ...
                for (uint32_t face = 0; face < 6; ++face)
                {
                    ...
                    vkCmdPushConstants(cmd, prefilterPipe.GetLayout(), VK_SHADER_STAGE_VERTEX_BIT,
                        0, sizeof(PushFace), &push);
                    vkCmdDraw(cmd, 3, 1, 0, 0);
                    vkCmdEndRenderPass(cmd);
                }
            ...
            vkCmdPushConstants(cmd, brdfPipe.GetLayout(), VK_SHADER_STAGE_VERTEX_BIT,
                0, sizeof(PushFace), &push);
            vkCmdDraw(cmd, 3, 1, 0, 0);
            vkCmdEndRenderPass(cmd);
        });
    ...
    void EnvironmentLighting::createColorImage(const Context& ctx, Image& image, uint32_t size,
        VkFormat format, uint32_t mipLevels, uint32_t layers)
    {
        image.Create(ctx, size, size, format,
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_IMAGE_ASPECT_COLOR_BIT,
            mipLevels, VK_SAMPLE_COUNT_1_BIT, layers,
            layers > 1 ? VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT : 0,
