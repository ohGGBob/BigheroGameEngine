...
namespace BigHero::Render
{
    ...
    {
    public:
        GpuBlockAllocator(VkDeviceSize capacity, VkDeviceSize minAlign)
            : capacity_(capacity), minAlign_(minAlign)
        {
    ...
    private:
        ...
        void Coalesce()
        {
            std::sort(free_.begin(), free_.end(),
                [](const FreeSpan& a, const FreeSpan& b) { return a.off < b.off; });
    ...
    {
    public:
        GpuAllocator(VkDeviceSize blockSize, VkDeviceSize minAlign,
                     uint32_t maxBlocks, CreateBlockFn createBlock)
            : blockSize_(blockSize), minAlign_(minAlign),
              maxBlocks_(maxBlocks), createBlock_(std::move(createBlock)) {}
