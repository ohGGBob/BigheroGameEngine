...
namespace BigHero
{
    namespace
    {
        ...
        void DecodeRleScanline(const uint8_t*& cur, const uint8_t* end,
            uint8_t* out, uint32_t width)
        {
            ...
            const bool isRle = (c0 == 0x02 && c1 == 0x02 && width >= 8 && width <= 0x7fff
                && ((static_cast<size_t>(c2) << 8) | c3) == width);
            if (!isRle)
        {
    ...
    void HdrImage::DecodeScanline(const uint8_t*& cur, const uint8_t* end,
        uint8_t* out, uint32_t width)
    {
    ...
    glm::vec3 HdrImage::SampleEquirect(const glm::vec3& dir,
        const glm::vec3* rgba, uint32_t width, uint32_t height)
    {
    ...
    std::array<std::vector<glm::vec3>, 6> HdrImage::EquirectToCube(
        const glm::vec3* rgba, uint32_t width, uint32_t height, uint32_t size)
    {
            ...
            for (uint32_t y = 0; y < size; ++y)
            {
                for (uint32_t x = 0; x < size; ++x)
                {
                    ...
                    const glm::vec3 dir = glm::normalize(
                        major + sVec * (2.0f * s - 1.0f) + tVec * (2.0f * t - 1.0f));
