...
namespace BigHero
{
    namespace
    {
        ...
        void UploadPixels(const Context& ctx, const void* pixels, uint32_t width, uint32_t height,
            VkDeviceSize byteSize, VkFormat format, Image& outImage, VkDevice device, VkSampler& outSampler)
        {
            ...
            {
                outImage.TransitionLayout(ctx, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    void Texture::CreateCheckerboard(const Context& ctx, uint32_t size, uint32_t cells)
    {
        ...
        UploadPixels(ctx, pixels.data(), size, size,
            static_cast<VkDeviceSize>(pixels.size()), VK_FORMAT_R8G8B8A8_SRGB,
