...
namespace BigHero::Render
{
    ...
    public:
        ...
        {
            if (!Active(frameIndex) || slot >= kPerFrame) return;
            vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                pool_[frameIndex], slot);
        }
        ...
        void Resolve(uint32_t frameIndex) noexcept
        {
            ...
            const VkResult res = vkGetQueryPoolResults(device_, pool_[frameIndex], 0, kPerFrame,
                data.size() * sizeof(uint64_t), data.data(), sizeof(uint64_t),
