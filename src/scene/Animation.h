...
namespace BigHero::Scene
{
    ...
    public:
        ...
        explicit AnimationPlayer(const GltfModel& model, size_t animIndex = 0)
            : model_(&model)
        {
    ...
    private:
        ...
        static void Locate(const std::vector<float>& times, float time,
                           int& k, float& u)
        {
            const int n = static_cast<int>(times.size());
            if (time <= times[0]) { k = 0; u = 0.0f; return; }
            if (time >= times[n - 1]) { k = n - 2; u = 1.0f; return; }
            int lo = 0, hi = n - 1;
            while (lo + 1 < hi)
            {
        ...
        static glm::quat QuatFromVec4(const glm::vec4& v)
        {
