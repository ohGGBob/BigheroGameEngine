...
namespace BigHero::Scene
{
    ...
    inline std::vector<MtlMaterial> ParseMtl(const std::string& text)
    {
        ...
        while (std::getline(in, line))
        {
            ...
            if (key == "Ka") { read3(current->ambient); }
            else if (key == "Kd") { read3(current->diffuse); }
            else if (key == "Ks") { read3(current->specular); }
            else if (key == "Ns") { as >> current->shininess; }
            else if (key == "d") { as >> current->opacity; }
            else if (key == "Tr")
            {
                float t; if (as >> t) current->opacity = 1.0f - t;
            }
            else if (key == "illum") { as >> current->illum; }
            else if (key == "map_Kd") { current->mapKd = args; }
            else if (key == "map_Ks") { current->mapKs = args; }
            else if (key == "map_Bump" || key == "bump") { current->mapBump = args; }
            // 其余指令忽略
        }
        return materials;
    }
    ...
    inline std::vector<SubMesh> GroupFacesByMaterial(
