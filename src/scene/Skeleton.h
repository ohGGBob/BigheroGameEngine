...
namespace BigHero::Scene
{
    ...
    public:
        ...
        void ComputeNodeMatricesWithPose(const std::vector<glm::vec3>& t,
                                         const std::vector<glm::quat>& r,
        ...
        // 外部姿态 -> 关节全局矩阵（索引即关节下标）。
        void ComputeGlobalJointMatricesWithPose(const std::vector<glm::vec3>& t,
                                                const std::vector<glm::quat>& r,
                                                const std::vector<glm::vec3>& s,
                                                std::vector<glm::mat4>& outGlobal) const
        {
        ...
        void ComputeSkinMatricesWithPose(const std::vector<glm::vec3>& t,
                                         const std::vector<glm::quat>& r,
                                         const std::vector<glm::vec3>& s,
            ...
            std::vector<glm::vec3>& normalsOut) const
        {
            ...
            SkinVerticesWith(skin, jointIndices, jointWeights, positionsIn, normalsIn,
                             positionsOut, normalsOut);
        }

        // CPU 蒙皮（外部给定皮肤矩阵）：避免重复计算，便于逐帧动画驱动复用同一批矩阵。
        void SkinVerticesWith(const std::vector<glm::mat4>& skin,
                              const std::vector<glm::u8vec4>& jointIndices,
                              const std::vector<glm::vec4>& jointWeights,
                              const std::vector<glm::vec3>& positionsIn,
                              const std::vector<glm::vec3>& normalsIn,
    ...
    private:
        void ExtractJointMatrices(const std::vector<glm::mat4>& nodeGlobal,
                                  std::vector<glm::mat4>& outGlobal) const
        {
        ...
        void BuildSkin(const std::vector<glm::mat4>& global,
                       std::vector<glm::mat4>& outSkin) const
        {
