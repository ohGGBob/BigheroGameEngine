...
namespace BigHero::Scene
{
    ...
    {
    public:
        explicit SkinnedMesh(const GltfModel& model)
            : model_(&model), skeleton_(model)
        {
            // 缓存绑定姿态几何（原始顶点位置/法线）
        ...
        void EvaluateBind(std::vector<glm::vec3>& outPos,
        ...
        void Evaluate(size_t animIndex, float time, bool loop,
        ...
        void EvaluatePose(const std::vector<glm::vec3>& t,
    ...
    private:
        ...
        {
            return skeleton_.HasSkin() && !model_->jointIndices.empty();
        }

        void ApplySkin(const std::vector<glm::mat4>& skin,
                       std::vector<glm::vec3>& outPos,
                       std::vector<glm::vec3>& outNormal) const
        {
            skeleton_.SkinVerticesWith(skin, model_->jointIndices, model_->jointWeights,
                                       positions_, normals_, outPos, outNormal);
        }
